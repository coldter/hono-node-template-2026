...
Operator-facing React SPA consuming `apps/admin-server`. TanStack Router, TanStack Query, Zustand, Tailwind.

## Critical Rules
- Keep server state in TanStack Query; avoid duplicating it in local stores.
- Use schema-backed forms (React Hook Form + Zod).
- Gate restricted routes/actions with permission guards.
